#!/usr/bin/env bash
# Ablation: HNSW "neighbors" parallelism — thread scaling (1, 2, 4, 8).
#
# Parallelises the inner loop over a candidate's M neighbours during beam
# search (each thread evaluates one neighbour's distance). Intended to show
# that fine-grained inner parallelism carries heavy OMP fork overhead relative
# to the tiny M×D work, resulting in slowdown rather than speedup.
#
# Usage:
#     bash scripts/ablate_neighbors.sh
#     DIM=100 N=1000000 QUERIES=500 scripts/ablate_neighbors.sh
...
mkdir -p results
CSV="results/ablate_neighbors_M${M}_ef${EF}.csv"
...
# Panel 1 — wall time
axes[0].plot(threads, hnsw_ms,  "o-", color="tab:red",  label="HNSW (neighbors)")
axes[0].plot(threads, brute_ms, "s--", color="tab:gray", label="Brute force")
axes[0].set_xlabel("Threads"); axes[0].set_ylabel("Wall time (ms)")
axes[0].set_title("Wall time vs Threads"); axes[0].legend()
axes[0].set_xticks(threads)
for x, y in zip(threads, hnsw_ms):
    axes[0].annotate(f"{y:.1f}", (x, y), textcoords="offset points", xytext=(0,6), ha="center", fontsize=8)

# Panel 2 — speedup vs serial (expected to stay ≤ 1 or degrade)
...
axes[1].axhline(1.0, color="tab:red", linewidth=0.8, linestyle=":")
axes[1].set_xlabel("Threads"); axes[1].set_ylabel("Speedup")
axes[1].set_title("Speedup vs Serial (P=1)\n(overhead dominates for small M)"); axes[1].legend()
axes[1].set_xticks(threads)
for x, y in zip(threads, s_serial):
    axes[1].annotate(f"{y:.2f}×", (x, y), textcoords="offset points", xytext=(0,6), ha="center", fontsize=8)

# Panel 3 — recall stability
...
fig.suptitle("HNSW Ablation — 'neighbors' parallelism (inner neighbour distances)", fontsize=13)
fig.tight_layout()
