#!/usr/bin/env bash
# Ablation: HNSW "queries" parallelism — thread scaling (1, 2, 4, 8).
#
# Measures how wall time and speedup change as more threads are used for the
# outer parallel-for over the query batch (static schedule).
#
# Usage:
#     bash scripts/ablate_queries.sh
#     DIM=100 N=1000000 QUERIES=500 scripts/ablate_queries.sh
#
# Env knobs: DIM, N, QUERIES, TOPK, M, EF_CONSTRUCTION, EF, THREADS, SOURCE
...
mkdir -p results
CSV="results/ablate_queries_M${M}_ef${EF}.csv"
...
# Panel 1 — wall time
axes[0].plot(threads, hnsw_ms,  "o-", color="tab:blue",  label="HNSW (queries)")
...
axes[1].set_title("Speedup vs Serial (P=1)"); axes[1].legend()
...
fig.suptitle("HNSW Ablation — 'queries' parallelism (outer query batch)", fontsize=13)
fig.tight_layout()
